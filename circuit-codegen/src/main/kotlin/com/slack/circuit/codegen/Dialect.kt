...
/**
 * A "dialect" of the codegen: the set of runtime types and naming rules that differ between the
 * `@CircuitInject` ([Circuit]) and `@SubCircuitInject` ([SubCircuit]) processors. Everything else —
 * DI-mode wiring, provider hoisting, qualifier/origin propagation, the `when (screen)` skeleton —
 * is shared and lives in the processor.
 *
 * A single [CircuitSymbolProcessor][com.slack.circuit.codegen.CircuitSymbolProcessorProvider] scans
 * both annotations and drives generation with the matching dialect per element.
 */
internal sealed class Dialect {
  /** The inject annotation this dialect scans and reads the `screen`/`scope` arguments from. */
  abstract val injectAnnotation: ClassName
  ...
  abstract val uiNoun: String
  /** Human-readable noun for the presenter type, used in diagnostics. */
  abstract val presenterNoun: String

  /** The UI factory interface the generated UI factory implements. */
  abstract val uiFactory: ClassName
  /** The presenter factory interface the generated presenter factory implements. */
  abstract val presenterFactory: ClassName

  /** Fallback UI-state type used for UI functions that omit a state parameter. */
  abstract val defaultUiState: ClassName

  /** The UI-state [KSType] used to detect a function's state parameter. */
  abstract fun uiState(symbols: CircuitSymbols): KSType

  /** Maps a supertype's qualified name to its [FactoryType], or null if it isn't a UI/presenter. */
  abstract fun factoryTypeForSuperType(qualifiedName: String): FactoryType?

  /** The [FactoryType] a top-level function produces. */
  abstract fun functionFactoryType(functionName: String): FactoryType

  /** Whether [type] is (or extends) this dialect's screen type. */
  abstract fun isScreenType(type: KSType, symbols: CircuitSymbols): Boolean

  /** Whether [type] is this dialect's navigator type. Only Circuit provides a navigator. */
  open fun isNavigatorType(type: KSType, symbols: CircuitSymbols): Boolean = false

  /** Whether [type] is this dialect's context type. Only Circuit provides a context. */
  open fun isContextType(type: KSType, symbols: CircuitSymbols): Boolean = false
   ...
   */
  abstract fun isProvidedFunctionParameter(
    type: KSType,
  ...
  /**
   * Whether this dialect provides assisted arguments (screen/navigator/context) to top-level
   * functions. Circuit does; SubCircuit functions only receive state and modifier.
   */
  abstract val providesAssistedParamsToFunctions: Boolean
   ...
   */
  abstract fun uiFunctionWrapperPrefix(): CodeBlock

  /** The parameters of the generated factory's `create` function for the given [factoryType]. */
  abstract fun createParams(factoryType: FactoryType): List<ParameterSpec>

  /** The (nullable) return type of the generated factory's `create` function. */
  abstract fun createReturn(factoryType: FactoryType): TypeName
  ...
  abstract fun classPackageName(
    annotatedDeclaration: KSClassDeclaration,
    targetClass: KSClassDeclaration,
  ): String

  /** The suffix appended to the base name to form the generated factory class name. */
  abstract fun factorySuffix(factoryType: FactoryType): String

  /** Diagnostic emitted when an annotated class isn't injectable. */
  abstract fun notInjectableError(): String

  object Circuit : Dialect() {
