...
private class CircuitSymbolProcessor(
  ...
  override fun process(resolver: Resolver): List<KSAnnotated> {
    ...
    // (@SubCircuitInject) dialects. Only the annotations present on the consumer's classpath match
    // anything, so a plain Circuit or SubCircuit consumer only ever generates its own factories.
    for (dialect in listOf(Dialect.Circuit, Dialect.SubCircuit)) {
      resolver.getSymbolsWithAnnotation(dialect.injectAnnotation.canonicalName).forEach {
        annotatedElement ->
        when (annotatedElement) {
          is KSClassDeclaration ->
            generateFactory(annotatedElement, InstantiationType.CLASS, symbols, dialect)

          is KSFunctionDeclaration ->
            generateFactory(annotatedElement, InstantiationType.FUNCTION, symbols, dialect)

          else ->
            logger.error(
              "${dialect.annotationSimpleName} is only applicable on classes and functions.",
              annotatedElement,
            )
        }
      }
  ...
  private fun generateFactory(
    ...
    val injectAnnotation =
      annotatedElement.getKSAnnotationsWithLeniency(dialect.injectAnnotation).single()
    ...
    if (instantiationType == InstantiationType.CLASS) {
      (annotatedElement as KSClassDeclaration).checkForAssistedInjection(mode, dialect) {
        return
      }
    }
    ...
    val factoryData =
      computeFactoryData(annotatedElement, symbols, screenKSType, instantiationType, dialect)
        ?: return
    ...
    val generatedClassName = baseName + dialect.factorySuffix(factoryData.factoryType)
  ...
  private inline fun KSDeclaration.checkVisibility(dialect: Dialect, returnBody: () -> Unit) {
    if (!getVisibility().isVisible) {
      logger.error(
        "${dialect.annotationSimpleName} is not applicable to private or local functions and " +
          "classes.",
        this,
      )
      returnBody()
  ...
  private inline fun KSClassDeclaration.checkForAssistedInjection(
    ...
    if (findConstructorAnnotatedWith(assistedInjectClassName) != null) {
      ...
      logger.error(
        "When using @${dialect.annotationSimpleName} with an @AssistedInject-annotated class, " +
          "you must put the @${dialect.annotationSimpleName} annotation on the " +
  ...
  private fun computeFactoryData(
    ...
    return when (instantiationType) {
      InstantiationType.FUNCTION ->
        computeFunctionFactoryData(annotatedElement, symbols, screenKSType, dialect)
      InstantiationType.CLASS ->
        computeClassFactoryData(annotatedElement, symbols, screenKSType, dialect)
    }
  }
  ...
  private fun computeFunctionFactoryData(
    ...
    for (param in fd.parameters) {
      ...
      if (dialect.isProvidedFunctionParameter(type, factoryType, symbols)) continue
    ...
    if (factoryType == FactoryType.PRESENTER) {
      ...
      val isInvalid = returnType == null || !returnType.isInstanceOf(dialect.uiState(symbols))
      if (isInvalid) {
        ...
        FactoryType.UI -> {
          ...
          val stateParam =
            fd.parameters.singleOrNull { parameter ->
              dialect.uiState(symbols).isAssignableFrom(parameter.type.resolve())
            }
          ...
          val modifierParam =
            fd.parameters.singleOrNull { parameter ->
              ...
              ?: run {
                logger.error(
                  "${dialect.uiNoun} composable functions must have a Modifier parameter!",
                  fd,
                )
                return null
              }
          ...
          */
          ...
          val stateType =
            if (stateParam == null) dialect.defaultUiState
          ...
          CodeBlock.of(
            ...
            dialect.uiFunctionWrapperPrefix(),
            stateType,
            stateArg,
            MemberName(packageName, name),
            stateParamBlock,
  ...
  private fun computeClassFactoryData(
    ...
    declaration.checkVisibility(dialect) {
      return null
    }
    val constructorParams = mutableListOf<ParameterSpec>()
    ...
    if (isAssisted) {
      if (mode == KOTLIN_INJECT_ANVIL) {
        ...
        targetClass.checkVisibility(dialect) {
          return null
        }
      }
    } else {
    ...
    if (!isAssisted && !useProvider) {
      logger.error(dialect.notInjectableError(), declaration)
      return null
    }
    val className =
      dialect.classBaseName(
        annotatedDeclaration = declaration,
        targetClass = targetClass,
        isAssisted = isAssisted,
        isKotlinInjectAnvil = mode == KOTLIN_INJECT_ANVIL,
      )
    val packageName = dialect.classPackageName(declaration, targetClass)
    val factoryType =
      targetClass
        .getAllSuperTypes()
        .mapNotNull {
          dialect.factoryTypeForSuperType(it.declaration.qualifiedName?.asString().orEmpty())
        }
        .firstOrNull()
        ?: run {
          ...
          logger.error(
            "Factory must be for a ${dialect.uiNoun} or ${dialect.presenterNoun} class, but was " +
  ...
  private fun KSFunctionDeclaration.assistedParameters(
    ...
    return buildSet {
        for (param in parameters) {
          ...
          val type = param.type.resolve()
          when {
            dialect.isScreenType(type, symbols) -> {
            ...
            dialect.isNavigatorType(type, symbols) -> {
              if (allowNavigator) {
                addOrError(
                  AssistedType(
            ...
            dialect.isContextType(type, symbols) -> {
              addOrError(
                AssistedType(
                  factoryName = "context",
                  type = type.toTypeName(),
...
/**
 * Builds the generated factory [TypeSpec] for either a UI or presenter, emitting the dialect's
...
private fun TypeSpec.Builder.buildFactory(
  ...
  return addSuperinterface(dialect.factorySuperinterface(factoryType))
    .addFunction(
      FunSpec.builder("create")
        .addModifiers(KModifier.OVERRIDE)
        .addParameters(dialect.createParams(factoryType))
        .returns(dialect.createReturn(factoryType))
