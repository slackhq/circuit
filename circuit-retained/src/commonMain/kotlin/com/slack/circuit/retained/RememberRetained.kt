...
public fun <T : Any> rememberRetained(vararg inputs: Any?, key: String? = null, init: () -> T): T =
  rememberRetainedSaveable(*inputs, saver = Saver({ null }, { null }), key = key, init = init)
 ...
 * You can use it with a value stored inside [androidx.compose.runtime.mutableStateOf].
 *
 * The retained value will also be persisted in saved instance state, via [saver].
 *
 * This differs from `rememberSaveable` by preferring to retain state in memory first, and restoring
 * from saved instance state second.
 *
 * @param inputs A set of inputs such that, when any of them have changed, will cause the state to
 *   reset and [init] to be rerun
 * @param saver A [Saver] for the value
 * @param key An optional key to be used as a key for the saved value. If not provided we use the
 *   automatically generated by the Compose runtime which is unique for the every exact code
 *   location in the composition tree
 * @param init A factory function to create the initial value of this state
...
public fun <T> rememberRetainedSaveable(
  ...
  rememberRetainedSaveable(*inputs, saver = mutableStateSaver(stateSaver), key = key, init = init)
